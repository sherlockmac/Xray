...
get_port() {
    ...
    while :; do
        ...
        if [[ $is_count -ge 233 ]]; then
            err "The number of failures to automatically obtain available ports reaches 233, please check the port occupancy."
        fi
...
is_port_used() {
    ...
    msg "Please execute: $(_yellow "${cmd} update -y; ${cmd} install net-tools -y") to fix this."
}
...
ask() {
    ...
    set_ss_method)
        ...
        is_opt_msg="\nPlease select an encryption method:\n"
    ...
    set_header_type)
        ...
        is_opt_msg="\nPlease select a camouflage type:\n"
        is_opt_input_msg="(default\e[92m $is_default_arg\e[0m):"
...
change() {
    ...
    [[ $is_dont_auto_exit ]] && {
        ...
        info $1
        [[ $is_auto_get_config ]] && msg "\nautomatically choose: $is_config_file"
    }
    is_old_net=$net
    ...
    1)
        # new port
        is_new_port=$3
        [[ $host && ! $is_caddy ]] && err "($is_config_file) Changing the port is not supported because it doesn't make sense."
        if [[ $is_new_port && ! $is_auto ]]; then
            [[ ! $(is_test port $is_new_port) ]] && err "Please enter the correct port, optional (1-65535)"
            [[ $is_new_port != 443 && $(is_test port_used $is_new_port) ]] && err "Unable to use ($is_new_port) port"
        fi
        [[ $is_auto ]] && get_port && is_new_port=$tmp_port
        [[ ! $is_new_port ]] && ask string is_new_port "Please enter a new port:"
    ...
    2)
        # new host
        is_new_host=$3
        [[ ! $host ]] && err "($is_config_file) Changing the domain name is not supported."
    ...
    3)
        ...
        [[ ! $is_new_path ]] && ask string is_new_path "Please enter a new path:"
    ...
    4)
        ...
        if [[ $net == 'ss' || $is_trojan || $is_socks_pass ]]; then
            ...
            err "($is_config_file) Changing password is not supported."
        fi
    ...
    5)
        # new uuid
        is_new_uuid=$3
        [[ ! $uuid ]] && err "($is_config_file) Changes are not supported UUID."
    ...
    7)
        ...
        [[ ! $header_type ]] && err "($is_config_file) Changing masquerade type is not supported."
        [[ $is_auto ]] && {
    ...
    9)
        ...
        [[ ! $is_new_door_port ]] && {
            ask string door_port "Please enter a new destination port:"
    ...
    10)
        ...
        if [[ $is_auto ]]; then
            ...
            if [[ $? != 0 ]]; then
                err "Private key failed the test."
            fi
    ...
    11)
        # new serverName
        is_new_servername=$3
        [[ ! $is_reality ]] && err "($is_config_file) Changes are not supported serverName."
    ...
    12)
        ...
        [[ ! $is_dynamic_port ]] && err "($is_config_file) Changing the dynamic port is not supported."
        if [[ $is_auto ]]; then
            get dynamic-port
            add $net
        else
            ...
            [[ ! $is_new_dynamic_port_start ]] && ask string is_new_dynamic_port_start "Please enter a new dynamic start port:"
    ...
    13)
        ...
        is_new_proxy_site=$3
        [[ ! $is_caddy && ! $host ]] && {
            err "($is_config_file) Changing the fake website is not supported."
        }
        [[ ! -f $is_caddy_conf/${host}.conf.add ]] && err "Unable to configure masquerading."
        [[ ! $is_new_proxy_site ]] && ask string is_new_proxy_site "Please enter a new fake site (eg example.com):"
        ...
        msg "\nUpdated fake website to: $(_green $proxy_site) \n"
        ;;
    14)
        # new kcp seed
        is_new_kcp_seed=$3
        [[ ! $kcp_seed ]] && err "($is_config_file) Changes are not supported mKCP seed."
...
uninstall() {
    ...
    _green "\nUninstall complete!"
    msg "Where does the script need to be improved? Please feedback"
    msg "Feedback issues) $(msg_ul https://github.com/${is_sh_repo}/issues)\n"
}
    ...
    [[ $is_test_run && ! $is_new_install ]] && {
        ...
        if [[ ! $(pgrep -f $is_run_bin) ]]; then
            ...
            [[ ! $is_no_manage_msg ]] && {
                ...
                _yellow "A run failure is detected, and the test run is executed automatically."
                get test-run
                _yellow "When the test is over, press Enter to exit."
            }
        fi
    }
}
...
api() {
    [[ ! $1 ]] && err "Unrecognized parameters for the API."
    [[ $is_core_stop ]] && {
        warn "$is_core_name is currently stopped."
        is_api_fail=1
        return
    }
    case $1 in
    ...
    [[ ! $is_api_port ]] && {
        ...
        [[ $? != 0 ]] && {
            warn "Failed to read API port, unable to use API operation."
            return
        }
    }
...
add() {
    ...
    *http)
    ...
    if [[ $is_no_auto_tls && ! $is_use_tls ]]; then
        err "$is_new_protocol Manual configuration of tls is not supported."
    fi
    ...
    if [[ $2 ]]; then
        ...
        if [[ $is_use_port ]]; then
            ...
            [[ $(is_test port_used $is_use_port) ]] && {
                err "Unable to use ($is_use_port) port."
            }
            port=$is_use_port
        fi
        ...
        if [[ $is_use_path ]]; then
            [[ ! $(is_test path $is_use_path) ]] && {
                err "($is_use_path) is not a valid path."
            }
            path=$is_use_path
        fi
        if [[ $is_use_header_type || $is_use_method ]]; then
            ...
            is_tmp_list=${ss_method_list[@]}
            [[ ! $is_use_method ]] && {
                is_tmp_use_name=Camouflage type
                ask set_header_type
            }
            ...
            [[ ! ${is_tmp_use_type} ]] && {
                warn "(${is_use_header_type}${is_use_method}) is not a valid ${is_tmp_use_name}."
    ...
    if [[ $is_use_tls ]]; then
        if [[ ! $is_no_auto_tls && ! $is_caddy ]]; then
            ...
            [[ $(is_test port_used 80) || $(is_test port_used 443) ]] && {
                warn "The port (80 or 443) is already in use, unable to complete auto-configuration of TLS. Consider using no-auto-tls"
                msg "\e[41mhelp (help)\e[0m: $(msg_ul https://233boy.com/$is_core/no-auto-tls/)\n"
                exit 1
            }
            is_install_caddy=1
        fi
        ...
        if [[ $is_main_start ]]; then
            ...
            # set port
            [[ ! $port ]] && ask string port "Please enter port:"
            ...
            *tcp* | *kcp* | *quic*)
            ...
            [[ $is_dynamic_port && ! $is_dynamic_port_range ]] && {
                ask string is_use_dynamic_port_start "Please enter dynamic start port:"
    ...
    if [[ $is_new_protocol == 'Dokodemo-Door' ]]; then
        ...
        # set remote port
        [[ ! $door_port ]] && ask string door_port "Please enter the destination port:"
    fi
    ...
    if [[ $(grep 2022 <<<$ss_method) ]]; then
        ...
        [[ $ss_password ]] && {
            ...
            $is_core_bin -test <<<"$is_new_json" &>/dev/null
            if [[ $? != 0 ]]; then
                warn "Shadowsocks protocol ($ss_method) does not support using password ($(_red_bg $ss_password))\n\nYou can use the command: $(_green $is_core ss2022) to generate a supported password.\n\nThe script will automatically create a usable password:)"
                ss_password=
                # create new json.
                json_str=
            fi
...
get() {
    ...
    file)
        ...
        [[ ! $is_all_json ]] && err "Could not find associated config file: $2"
    ...
    protocol)
        ...
        *reality*)
            ...
            ;;
        *)
            err "Unrecognized transport protocol: $is_config_file"
            ;;
        esac
        ;;
    ...
    dynamic-port-test) # test dynamic port
        ...
        [[ $(is_test port_used $is_use_dynamic_port_end) ]] && {
            err "Dynamic port ($is_use_dynamic_port_start-$is_use_dynamic_port_end), but ($is_use_dynamic_port_end) port cannot be used."
        }
        [[ $is_use_dynamic_port_end -le $is_use_dynamic_port_start ]] && {
            err "Could not handle dynamic port ($is_use_dynamic_port_start-$is_use_dynamic_port_end) range correctly."
        }
        [[ $is_use_dynamic_port_start == $port || $is_use_dynamic_port_end == $port ]] && {
            err "Dynamic port ($is_use_dynamic_port_start-$is_use_dynamic_port_end) range conflicts with main port ($port)."
        }
...
url_qr() {
    ...
    if [[ $is_url ]]; then
        ...
        [[ $1 == 'url' ]] && {
            err "($is_config_name) cannot generate URL link."
        } || {
            err "($is_config_name) cannot generate QR code."
        }
    fi
}
...
update() {
    ...
    *)
    ...
    [[ $is_run_ver == $is_new_ver ]] && {
        msg "\nThe custom version is the same as the current $is_show_name version, no update is required.\n"
        exit
    }
    load download.sh
    if [[ $is_new_ver ]]; then
        ...
        [[ $is_run_ver == $latest_ver ]] && {
            msg "\n$is_show_name it's currently the latest version.\n"
            exit
        }
        msg "\nNew version of $is_show_name found: $(_green $latest_ver)\n"
    ...
    msg "$(_green Please check the update instructions: https://github.com/$is_update_repo/releases/tag/$is_new_ver)\n"
...
is_main_menu() {
    ...
    8)
        ...
        ;;
    9)
        ask list is_do_other "Enable BBR View Log View Error Log Test Run Reload Script"
        case $REPLY in
        1)
            load bbr.sh
            _try_enable_bbr
...
main() {
    ...
    dd | ddel | fix | fix-all)
        ...
        *)
            is_dont_auto_exit=1
            [[ ! $2 ]] && {
                err "Unable to find parameter to remove"
            } || {
                for v in ${@:2}; do
                    del $v
                done
